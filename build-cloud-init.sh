...
docker stop cloudinitpxecom-cloud-init 2>/dev/null || true
docker rm cloudinitpxecom-cloud-init 2>/dev/null || true
...
docker build \
  ...
  -t cloudinitpxecom-cloud-init:latest \
  .
...
echo "  docker run -d \\"
echo "    --name cloudinitpxecom-cloud-init \\"
echo "    -p 3000:3000 \\"
echo "    -p 69:69/udp \\"
echo "    -p 8080:80 \\"
...
echo "    cloudinitpxecom-cloud-init:latest"
echo ""
